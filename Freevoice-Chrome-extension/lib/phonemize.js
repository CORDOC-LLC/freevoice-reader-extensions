...
// Multi-language phonemizer (loaded on demand for non-English languages)
let multiLangPhonemizer = null;

// Load multi-language phonemizer on demand
async function getMultiLangPhonemizer() {
  if (multiLangPhonemizer) return multiLangPhonemizer;
  const module = await import("./espeakng-multi/phonemizer.js");
  multiLangPhonemizer = module.phonemize;
  return multiLangPhonemizer;
}

// Map Kokoro voice language codes to espeak-ng identifiers
const LANG_MAP = {
  "a": "en-us",     // American English (default)
...
// Check if a language requires multi-language phonemizer
function needsMultiLang(language) {
  return language && !["a", "b"].includes(language);
}
...
export async function phonemize(text, language = "a", norm = true) {
  ...
  // Use multi-language phonemizer for non-English languages
  const espeakLang = LANG_MAP[language] || "en-us";
  const useMultiLang = needsMultiLang(language);

  let phonemizer = espeakng;
  if (useMultiLang) {
    phonemizer = await getMultiLangPhonemizer();
  }

  const ps = (await Promise.all(sections.map(async ({ match, text }) => {
    if (match) return text;
    const phonemes = await phonemizer(text, espeakLang);
    return phonemes.join(" ");
  }))).join("");

  // 4. Post-process phonemes (only for English)
  let processed = ps;
  if (!useMultiLang) {
    processed = ps
