...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function init() {
    // Load saved settings
    const settings = await chrome.storage.sync.get(['selectedVoice', 'playbackSpeed', 'onboardingComplete']);
  ...
  async function showReadyState() {
    ...
    if (tab && tab.id) {
      try {
        ...
        // Can't execute script on this page (chrome://, etc)
  ...
  function setupOnboarding() {
    ...
    onboardingDownload.addEventListener('click', () => {
        ...
        chrome.runtime.sendMessage({ type: "GET_STATUS" }, (response) => {
          ...
          if (response) {
            if (response.isModelLoaded) {
              ...
              // Show different text for cached model (loads much faster)
      ...
      }, 300); // Poll faster for cached model scenario
    });
  ...
  readPageBtn.addEventListener('click', async () => {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (tab) {
      chrome.tabs.sendMessage(tab.id, { type: "READ_ARTICLE" });
      window.close();
    }
  });
  ...
  readTextBtn.addEventListener('click', async () => {
    ...
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (tab) {
      chrome.tabs.sendMessage(tab.id, { type: "READ_TEXT", text });
      window.close();
    }
  });
