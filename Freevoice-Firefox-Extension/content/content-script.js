// FreeVoice Reader - Content Script (Firefox)
...
(function() {
  ...
  let playbackMode = 'streaming';
  let currentSessionMode = 'streaming';
  ...
  // Load saved settings
  browser.storage.sync.get(['selectedVoice', 'volume', 'playbackSpeed', 'playerPosition', 'playbackMode']).then((result) => {
  ...
  function setupPlayerEvents() {
    ...
    modeStreamingBtn.addEventListener('click', () => {
      if (currentSessionMode === 'streaming') return;
      switchPlaybackMode('streaming');
    });

    modeFullBtn.addEventListener('click', () => {
      if (currentSessionMode === 'full') return;
    ...
    if (speedSelect) {
      ...
      speedSelect.addEventListener('change', (e) => {
        ...
        browser.storage.sync.set({ playbackSpeed: playbackSpeed });
      });
    }
  }
  ...
  function switchPlaybackMode(newMode) {
    ...
    if (wasGenerating || wasPlaying) {
      stopPlayback();
      browser.runtime.sendMessage({ type: "STOP_GENERATION" });
      isGenerating = false;
  ...
  // Rebuild audio queue from all chunks
  ...
  function startGeneration(text) {
    ...
    browser.runtime.sendMessage({
      ...
      console.error("[FreeVoiceReader] GENERATE_SPEECH error:", err);
    });
  }
  ...
  // Create HTMLAudioElement from all audio chunks
